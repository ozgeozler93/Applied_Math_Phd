...
example (hf : Function.Injective f) (hg : Function.Injective g) : Function.Injective (g ∘ f) := by
  intro x y hcomp
...
example (A : Set α) (B : Set α) (x : α) : x ∈ A ∩ B → x ∈ A := by
  intro h
  exact h.left
...
-/
theorem LimitAt.const {c a :ℝ} : LimitAt (fun _ : ℝ => c) a c := by
  intro ε hε
